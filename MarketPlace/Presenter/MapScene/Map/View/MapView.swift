...
struct MapView: View {
    ...
    var body: some View {
        NavigationView {
                ...
                KakaoMapView(
                    ...
                    .task(id: selectedCategory) {
                        await viewModel.fetchMarketsWithAddress(
                            lastPageIndex: nil,
                            category: MarketCategory(index: selectedCategory)?.apiValue ?? nil,
                            pageSize: 40
                        )
                    }
            ...
            .task(id: selectedCategory, {
                await viewModel.fetchMarkets(category: MarketCategory(index: selectedCategory)?.apiValue ?? nil)
                await viewModel.fetchMarketsWithAddress(lastPageIndex: nil, category: MarketCategory(index: selectedCategory)?.apiValue ?? nil, pageSize: nil)
            })
