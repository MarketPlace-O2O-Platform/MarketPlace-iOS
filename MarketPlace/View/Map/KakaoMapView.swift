...
struct KakaoMapView: UIViewRepresentable {
    ...
    func makeCoordinator() -> KakaoMapCoordinator {
        return KakaoMapCoordinator(location: locationManager, pois: pois)
    }
    ...
    class KakaoMapCoordinator: NSObject, MapControllerDelegate {
        ...
        init(location: LocationManager, pois: [KakaoMapPoi]) {
            self.pois = pois
        ...
        func addViews() {
            let defaultPosition: MapPoint = MapPoint(longitude: locationManager.region.coordinate.longitude, latitude: locationManager.region.coordinate.latitude)
        ...
        func containerDidResized(_ size: CGSize) {
            ...
            if first {
                let cameraUpdate = CameraUpdate.make(
                    target: MapPoint(
                        longitude: locationManager.region.coordinate.longitude,
                         latitude: locationManager.region.coordinate.latitude),
                    zoomLevel: 15,
                    mapView: mapView!
                )
